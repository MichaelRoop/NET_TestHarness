...
namespace LogUtils.Net {
    ...
    public static class MsgLevelExtensions {
        ...
        public static MsgLevel ToMsgLevel(this int level) {
            //return EnumTools.GetEnumFromInt<MsgLevel>(zone, MsgLevel.Info);
            try {
                return (MsgLevel)Enum.ToObject(typeof(MsgLevel), level);
            }
            catch (Exception) {
